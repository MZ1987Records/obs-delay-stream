...
PropertiesBuilder::PropertiesBuilder(obs_properties_t* props, DelayStreamData* d)
    : props_(props), d_(d) {}
...
// ---------- Static callbacks (properties-ui 由来) ----------

bool PropertiesBuilder::cb_rtmp_url_auto_changed(void* priv, obs_properties_t* props,
...
bool PropertiesBuilder::cb_ws_server_start(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
...
bool PropertiesBuilder::cb_ws_server_stop(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
    if (!d || !d->router_running.load()) return false;
    d->router.stop();
...
bool PropertiesBuilder::cb_tunnel_start(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
    if (!d || !d->context) return false;
...
bool PropertiesBuilder::cb_tunnel_stop(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
    if (!d) return false;
    d->tunnel.stop();
...
bool PropertiesBuilder::cb_flow_start(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
    if (!d) return false;
...
bool PropertiesBuilder::cb_flow_retry_failed(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
    if (!d) return false;
...
bool PropertiesBuilder::cb_flow_reset(obs_properties_t*, obs_property_t*, void* priv) {
    auto* d = static_cast<DelayStreamData*>(priv);
    if (!d) return false;
    d->flow.reset();
...
bool PropertiesBuilder::cb_stream_id_changed(void*, obs_properties_t* props,
                                              obs_property_t*, obs_data_t* settings) {
...
bool PropertiesBuilder::cb_audio_codec_changed(void* priv, obs_properties_t* props,
                                                obs_property_t*, obs_data_t* settings) {
    auto* d = static_cast<DelayStreamData*>(priv);
...
void PropertiesBuilder::add_flow_rtmp_measure_section(obs_properties_t* grp) {
    if (!grp || !d_) return;

    FlowPhase phase = d_->flow.phase();
    FlowResult res  = d_->flow.result();
    ...
    obs_properties_add_color_button_row(
        grp, "flow_rtmp_measure_controls_row", T_("FlowRtmpMeasureLabel"),
...
void PropertiesBuilder::build_flow_panel(obs_properties_t* grp) {
    if (!grp || !d_) return;
    ...
    obs_properties_add_color_button_row(
        grp, "flow_measure_controls_row", T_("FlowMeasureLabel"), flow_measure_buttons,
    ...
    if (is_step1_measuring) {
        const int pct = (res.ping_total_count > 0)
            ...
            : 0;
        obs_properties_add_flow_progress(grp, "flow_s1_status",
            ...
            for (int i = 0; i < sub_count; ++i) {
                ...
                if (res.channels[i].measured) {
                    ...
                    snprintf(line, sizeof(line), "\n  Ch.%d %s : %.1f ms",
                             i + 1, name.c_str(), d_->sub[i].delay_ms);
    ...
    auto* retry_failed_btn = obs_properties_add_button2(
        grp, "flow_retry_btn", T_("FlowRetryFailed"), cb_flow_retry_failed, d_);
...
void PropertiesBuilder::add_plugin_group() {
    if (!props_ || !d_) return;
    ...
    if (d_->update_check.status.load(std::memory_order_acquire) ==
        UpdateCheckStatus::UpdateAvailable) {
        const std::string latest_version = d_->update_check.latest_version();
    ...
    if (d_->is_duplicate_instance) {
        ...
        if (try_get_parent_audio_sync_offset_ns(d_, sync_offset_ns) &&
    ...
    obs_properties_add_group(props_, "grp_plugin", T_("Plugin"), OBS_GROUP_NORMAL, grp);
}

void PropertiesBuilder::add_stream_group() {
    if (!props_ || !d_) return;
    obs_properties_t* grp = obs_properties_create();
    obs_property_t* sid_p =
        obs_properties_add_text(grp, "stream_id", T_("StreamId"), OBS_TEXT_DEFAULT);
    obs_property_set_modified_callback2(sid_p, cb_stream_id_changed, d_);
    obs_property_set_enabled(sid_p, false);
    {
        char info[128];
        snprintf(info, sizeof(info), T_("AutoIpFmt"), d_->auto_ip.c_str());
    ...
    if (d_->router_running.load()) {
        obs_property_set_enabled(sid_p, false);
        obs_property_set_enabled(ip_p, false);
        obs_property_set_enabled(port_p, false);
    }
    obs_properties_add_group(props_, "grp_stream", T_("GroupStreamId"), OBS_GROUP_NORMAL, grp);
}

void PropertiesBuilder::add_ws_group(bool has_sid) {
    if (!props_ || !d_) return;
    bool ws_running = d_->router_running.load();
    int ws_port = ws_running
        ? (int)d_->router.port()
        : d_->ws_port.load(std::memory_order_relaxed);
    ...
    if (ws_running && !d_->ws_send_enabled.load()) {
    ...
    obs_property_set_modified_callback2(codec_p, cb_audio_codec_changed, d_);
    ...
    uint32_t input_sr = d_->sample_rate > 0 ? d_->sample_rate : 48000;
    ...
    obs_properties_add_group(props_, "grp_ws", ws_title.c_str(), OBS_GROUP_NORMAL, grp);
}

void PropertiesBuilder::add_tunnel_group() {
    if (!props_ || !d_) return;
    TunnelState ts = d_->tunnel.state();
    std::string turl = d_->tunnel.url();
    std::string terr = d_->tunnel.error();
    ...
    bool cloudflared_downloading = d_->tunnel.cloudflared_downloading();
    bool ws_running = d_->router_running.load();
    ...
    obs_properties_add_group(props_, "grp_tunnel", tunnel_title, OBS_GROUP_NORMAL, grp);
}

void PropertiesBuilder::add_flow_group() {
    if (!props_ || !d_) return;
    obs_properties_t* grp = obs_properties_create();
    {
        ...
        if (d_->flow.phase() != FlowPhase::Idle) {
            obs_property_set_enabled(ping_count_p, false);
        }
    }
    build_flow_panel(grp);
    if ((obs_get_version() >> 24) < 32) {
        obs_property_t* ver_warn_p = obs_properties_add_text(
            grp, "obs_version_warning", T_("ObsVersionWarning"), OBS_TEXT_INFO);
        obs_property_text_set_info_word_wrap(ver_warn_p, true);
    }
    int64_t sync_offset_ns = 0;
    if (try_get_parent_audio_sync_offset_ns(d_, sync_offset_ns) &&
        sync_offset_ns != REQUIRED_AUDIO_SYNC_OFFSET_NS) {
        obs_property_t* warn_p = obs_properties_add_text(
            grp, "audio_sync_offset_warning", T_("AudioSyncOffsetWarning"), OBS_TEXT_INFO);
        obs_property_text_set_info_word_wrap(warn_p, true);
    }
    int active_channels = d_->sub_ch_count;
    int connected_channels = 0;
    for (int i = 0; i < active_channels; ++i) {
        if (d_->router.client_count(i) > 0) ++connected_channels;
    }
    char flow_title[192];
    snprintf(flow_title, sizeof(flow_title), T_("GroupSyncFlowWithConn"),
             T_("GroupSyncFlow"), connected_channels, active_channels);
    obs_properties_add_group(props_, "grp_flow", flow_title, OBS_GROUP_NORMAL, grp);
}

void PropertiesBuilder::add_master_group() {
    if (!props_ || !d_) return;
    obs_properties_t* grp = obs_properties_create();
    bool auto_mode = true;
    ...
    obs_property_set_modified_callback2(auto_p, cb_rtmp_url_auto_changed, d_);
    ...
    add_flow_rtmp_measure_section(grp);
