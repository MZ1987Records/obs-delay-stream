...
bool PropertiesBuilder::cb_rtmp_url_auto_changed(void *priv, obs_properties_t *props, obs_property_t *, obs_data_t *settings) {
...
bool PropertiesBuilder::cb_stream_id_changed(void *, obs_properties_t *props, obs_property_t *, obs_data_t *settings) {
		...
		if (is_step1_done_or_later) {
			...
			for (int i = 0; i < sub_count; ++i) {
				...
				if (res.channels[i].measured) {
					char line[192];
					snprintf(line, sizeof(line), "\n  Ch.%d %s : %.1f ms", i + 1, name.c_str(), d_->sub[i].delay_ms);
