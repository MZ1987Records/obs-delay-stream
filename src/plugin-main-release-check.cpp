...
bool query_status_code(HINTERNET h, DWORD& out_status) {
    ...
    return HttpQueryInfoA(h, HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER,
                          &out_status, &size, nullptr) == TRUE;
}
...
bool query_header_string(HINTERNET h, DWORD query, std::string& out) {
    ...
    size_t begin = 0;
    while (begin < buf.size() && std::isspace((unsigned char)buf[begin])) ++begin;
