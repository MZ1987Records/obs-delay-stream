...
namespace ods::core {
	...
	/**
	 * サンプルレート・チャンネル数に対応したリングバッファディレイ器。
	 *
	...
	public:
		...
		/// バッファを初期化する。
		/// @param max_delay_ms 確保する最大ディレイ量 (ms)
		...
		/// 音声をディレイ処理する。`input` / `output` は interleaved float32。
	...
	private:
		...
		uint32_t            delay_ms_    = 0;     ///< 現在のディレイ量 (ms)
		std::atomic<size_t> delay_samples_{0};    ///< ディレイ量をサンプル数に換算した値（atomic でクロススレッド更新）
