...
public:
    DelayTableWidget(obs_source_t* source,
        ...
        , ch_count_(static_cast<int>(channels.size()))
        , selected_ch_(selected_ch >= 0 && selected_ch < static_cast<int>(channels.size())
                           ? selected_ch : 0)
    {
        ...
        connect(table_, &QTableWidget::itemSelectionChanged,
                this,   &DelayTableWidget::onSelectionChanged);
            ...
            QStringLiteral("padding-left: 3px; padding-right: 3px;"));
        {
            const int w = QFontMetrics(reset_btn->font())
                              .horizontalAdvance(QStringLiteral("Reset")) + 20;
        ...
        connect(spin_, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
                this,  &DelayTableWidget::onValueChanged);
...
private:
    ...
    // delay_table_selected_ch は ds_update に影響しないため UI 再構築は起きない。
    void saveSelectedChannel()
    {
        if (!source_) return;
        obs_data_t* s = obs_source_get_settings(source_);
        if (!s) return;
        obs_data_set_int(s, "delay_table_selected_ch",
                         static_cast<long long>(selected_ch_));
        obs_source_update(source_, s);
        obs_data_release(s);
    }

    // スピン値が変わったとき: OBS 設定の sub{N}_adjust_ms を更新する。
    // ds_update が effective_delay_changed を検出してテーブルを再構築する。
...
void do_delay_table_inject(void* param)
{
    ...
    for (const auto& ph : found) {
        ...
        auto* widget = new DelayTableWidget(
            ctx->source, selected_ch, channels, labels, parent);
