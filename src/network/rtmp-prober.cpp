...
namespace ods::network {
	...
	std::string RtmpProber::host() const {
		return host_;
	}
	int RtmpProber::port() const {
		return port_;
	}
	...
	bool RtmpProber::parse_url(const std::string &url, std::string &host, int &port) {
		...
		for (auto prefix : {"rtmps://", "rtmp://"}) {
			if (url_str.rfind(prefix, 0) == 0) {
				url_str = url_str.substr(std::strlen(prefix));
				break;
			}
		}
		...
		if (slash != std::string::npos) url_str = url_str.substr(0, slash);

		// host:port 分割
		auto colon = url_str.rfind(':');
		if (colon != std::string::npos) {
			host = url_str.substr(0, colon);
			port = std::atoi(url_str.c_str() + colon + 1);
	...
	double RtmpProber::probe_once() {
		...
		SocketGuard sg;
		sg.sock = socket(ai.p->ai_family, ai.p->ai_socktype, ai.p->ai_protocol);
		if (sg.sock == INVALID_SOCKET) return -1.0;

		// タイムアウト設定
		DWORD timeout_ms = SOCKET_TIMEOUT_MS;
		setsockopt(sg.sock, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char *>(&timeout_ms), sizeof(timeout_ms));
		setsockopt(sg.sock, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char *>(&timeout_ms), sizeof(timeout_ms));

		// --- TCP接続（ここがTCP RTTを含む） ---
		auto t0 = ProbeClk::now();
		if (connect(sg.sock, ai.p->ai_addr, (int)ai.p->ai_addrlen) != 0)
			return -1.0;
		...
		if (send(sg.sock, reinterpret_cast<const char *>(c0c1), sizeof(c0c1), 0) != sizeof(c0c1))
			return -1.0;
		...
		while (received < target) {
			int n = recv(sg.sock, reinterpret_cast<char *>(s_buf + received), target - received, 0);
			if (n <= 0) break;
			received += n;
		}
	...
	void RtmpProber::probe_loop() {
		...
		if (samples.empty()) {
			...
			r.avg_rtt_ms     = std::accumulate(samples.begin(), samples.end(), 0.0) / r.samples;
