...
double RtmpProber::probe_once() {
	...
	setsockopt(sg.s, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<const char *>(&timeout_ms), sizeof(timeout_ms));
	setsockopt(sg.s, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<const char *>(&timeout_ms), sizeof(timeout_ms));
	...
	if (send(sg.s, reinterpret_cast<const char *>(c0c1), sizeof(c0c1), 0) != sizeof(c0c1))
		return -1.0;
	...
	while (received < target) {
		int n = recv(sg.s, reinterpret_cast<char *>(s_buf + received), target - received, 0);
		if (n <= 0) break;
		received += n;
	}
